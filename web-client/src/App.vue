...
const roomPins = ref([]), pinsExpanded = ref(false);
...
<template>
  ...
  <main v-else class="chat"><aside><header class="brand"><img :src="icon"><span>PolyChat<small>让交流保持简单</small></span></header><button class="new" @click="newRoom"><span>＋</span> 新建聊天室</button><p class="nav-label">聊天室</p><nav><button v-for="item in rooms" :key="item.id" :class="{active: room?.id === item.id, hasUnread: unread[item.id]}" @click="choose(item)"><span>#</span><b>{{ item.name }}</b><small v-if="unread[item.id]" class="unread">{{ unread[item.id] > 99 ? '99+' : unread[item.id] }}</small></button></nav><footer><button class="profile-button" title="更换头像" @click="profileOpen = true"><img v-if="avatar(user)" :src="avatar(user)"><b v-else>{{ user.username[0] }}</b></button><span>{{ user.username }}<small>{{ isAdmin ? '管理员 · 在线' : '在线' }}</small></span><button class="logout" title="退出登录" @click="logout">↪</button></footer></aside>
    <section class="conversation">
      <header class="topbar"><div><h2><span>#</span> {{ room?.name || '大厅' }} <small v-if="room?.is_private">🔒 私有</small></h2><small><i class="online-dot"></i>{{ onlineUsers.length }} 人在线<span v-if="typingText"> · {{ typingText }}</span></small></div><button class="toolbar-button" @click="loadPins">⌖ <em>置顶</em></button><button class="toolbar-button" @click="searchOpen = true">⌕ <em>搜索</em></button><button v-if="room?.is_private && (room?.role === 'owner' || room?.role === 'admin' || isAdmin)" class="toolbar-button" @click="loadMembers">♙ <em>成员</em></button><button v-if="isAdmin || room?.role === 'owner' || room?.role === 'admin'" class="toolbar-button" @click="openAnnouncement">📢 <em>公告</em></button><button v-if="isAdmin || room?.role === 'owner' || room?.role === 'admin'" class="toolbar-button" @click="openRoomManage">⚙ <em>房间</em></button><button class="toolbar-button" title="主题与自定义 CSS" @click="themeOpen = true"><span>◐</span><em>主题</em></button><button v-if="isAdmin" class="toolbar-button" @click="adminOpen = true; loadAdmin()">管理面板</button><button class="toolbar-button notification" :class="{on: notificationOn, blocked: notificationPermission === 'denied'}" :title="notificationLabel" @click="toggleNotifications"><span>{{ notificationOn ? '🔔' : '🔕' }}</span><em>{{ notificationButtonText }}</em></button></header>
      <div v-if="room?.announcement" class="announcement-banner"><div class="announcement-header"><span class="announcement-icon">📢</span><span>公告</span><small>by {{ room.announcement_username }}</small><div v-if="isAdmin || room?.role === 'owner' || room?.role === 'admin'" class="announcement-actions"><button @click="openAnnouncement">编辑</button><button @click="deleteAnnouncement">删除</button></div></div><div class="announcement-content markdown" v-html="markdown(room.announcement)"></div></div>
      <div v-if="roomPins.length" class="pinned-sticky"><div class="pinned-sticky-header" @click="pinsExpanded = !pinsExpanded"><span class="pinned-icon">📌</span><span>置顶消息 ({{ roomPins.length }})</span><span class="pinned-toggle">{{ pinsExpanded ? '收起' : '展开' }}</span></div><div v-if="pinsExpanded" class="pinned-sticky-list"><article v-for="pin in roomPins" :key="pin.id" class="pinned-sticky-item"><div class="pinned-sticky-meta"><strong>{{ pin.username }}</strong><small>{{ time(pin.created_at) }}</small><button v-if="isAdmin || room?.role === 'owner' || room?.role === 'admin'" class="pinned-unpin" @click="unpinMessage(pin)">取消置顶</button></div><div v-if="pin.content" class="markdown" v-html="markdown(pin.content)"></div></article></div></div>
  ...
  <div v-if="announcementOpen" class="modal"><section><button class="close" @click="announcementOpen = false">×</button><p>ANNOUNCEMENT</p><h2>房间公告</h2><textarea v-model="announcementDraft" rows="5" placeholder="输入公告内容…"></textarea><div class="theme-actions"><button class="primary" @click="saveAnnouncement">保存</button><button @click="announcementOpen = false">取消</button></div></section></div>
