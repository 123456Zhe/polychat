...
const retractedOriginal = ref(null), retractedOriginalOpen = ref(false);
...
const roomQuickMenuOpen = ref(false), moreMenuOpen = ref(false);
...
const draftExpand = ref(false), draftPreview = ref(false);
...
watch(content, (val) => { clearTimeout(draftSaveTimer); draftSaveTimer = setTimeout(() => { if (room.value) saveDraft('room', room.value.id, val); }, 500); });
watch(dmInput, (val) => { clearTimeout(draftSaveTimer); draftSaveTimer = setTimeout(() => { if (conversation.value) saveDraft('dm', conversation.value.id, val); }, 500); });
...
async function copyText(value, success = '已复制') {
  let ok = false;
  try { if (navigator.clipboard?.writeText) { await navigator.clipboard.writeText(value); ok = true; } } catch { /* insecure context or denied: fall through */ }
  if (!ok) {
    // 明文 HTTP（非 localhost）下 navigator.clipboard 不存在，回退到临时 textarea + execCommand
    try {
  ...
  if (ok) return notify(success);
  prompt('复制失败，请手动复制：', value);
}
...
async function loadPluginClientAssets() {
  try {
    const { assets } = await api('/api/plugins/client-assets');
    if (!assets || !assets.length) return;
    const loaded = new Set();
    for (const plugin of assets) {
      // 加载 CSS
      for (const cssUrl of plugin.css || []) {
        if (loaded.has(cssUrl)) continue;
      ...
      for (const jsUrl of plugin.js || []) {
        if (loaded.has(jsUrl)) continue;
        const script = document.createElement('script');
        script.src = jsUrl; script.defer = true;
        document.body.appendChild(script);
        loaded.add(jsUrl);
      }
    }
  } catch { /* 插件资产加载失败不影响主应用 */ }
}
...
// 插件状态：停用的插件（如 onebot / announcement）对应 UI 应禁用；管理员可热管理（安装/启停/卸载）
...
async function loadPlugins() {
  try {
    pluginsList.value = (await api('/api/plugins')).plugins;
    enabledPlugins.value = pluginsList.value.filter(p => p.enabled).map(p => p.name);
  } catch { pluginsList.value = []; enabledPlugins.value = []; }
}
function pluginEnabled(name) { return enabledPlugins.value.includes(name); }
async function setPluginEnabled(name, enabled) {
  try {
    await api(`/api/admin/plugins/${encodeURIComponent(name)}/enabled`, { method: 'PATCH', body: JSON.stringify({ enabled }) });
    notify(`${enabled ? '已启用' : '已停用'}插件 ${name}`);
    await loadPlugins();
  } catch (e) { notify(e.message); }
}
async function uninstallPlugin(name) {
  if (!confirm(`确定卸载插件 ${name}？${uninstallDeleteConfig.value ? '将同时删除其配置。' : '插件配置将保留。'}`)) return;
  try {
    await api(`/api/admin/plugins/${encodeURIComponent(name)}`, { method: 'DELETE', body: JSON.stringify({ delete_config: uninstallDeleteConfig.value }) });
    notify(`已卸载插件 ${name}`);
    await loadPlugins();
  } catch (e) { notify(e.message); }
}
async function installPluginFromUrl() {
  ...
  if (!url) return notify('请输入插件地址');
  installBusy.value = true;
  try {
    await api('/api/admin/plugins/install', { method: 'POST', body: JSON.stringify({ url }) });
    notify('插件安装成功（已热加载）');
    installUrl.value = '';
    await loadPlugins();
  } catch (e) { notify(e.message); } finally { installBusy.value = false; }
}
async function installPluginUpload(file) {
  if (!file) return;
  installBusy.value = true;
  try {
    const response = await fetch(`/api/admin/plugins/install/upload?filename=${encodeURIComponent(file.name)}`, { method: 'POST', body: await file.arrayBuffer() });
    const body = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(body.error || `请求失败 (${response.status})`);
    notify('插件上传安装成功（已热加载）');
    await loadPlugins();
  } catch (e) { notify(e.message); } finally { installBusy.value = false; }
}
async function loadMarketPlugins() {
  try { marketPlugins.value = (await api('/api/admin/plugins/market')).plugins; } catch (e) { notify(e.message); }
}
async function installFromMarket(plugin) {
  installBusy.value = true;
  try {
    await api('/api/admin/plugins/install', { method: 'POST', body: JSON.stringify({ url: plugin.repo }) });
    notify(`插件 ${plugin.name} 安装成功（已热加载）`);
    await loadPlugins(); await loadMarketPlugins();
  } catch (e) { notify(e.message); } finally { installBusy.value = false; }
}
...
function connectSocket() {
  ...
  socket.onopen = async () => { socketBackoff = 1000; connectionState.value = 'connected'; await loadRooms(); await pollNewMessages(); startPolling(); };
  socket.onmessage = message => { try { handleSocketEvent(JSON.parse(message.data)); } catch { /* ignore malformed frames */ } };
  socket.onclose = () => { socket = null; connectionState.value = 'reconnecting'; startPolling(); clearTimeout(reconnectTimer); reconnectTimer = setTimeout(connectSocket, socketBackoff); socketBackoff = Math.min(socketBackoff * 2, 30_000); };
...
async function choose(item) {
  ...
  saveDraft('room', room.value?.id, content.value);
  ...
  content.value = loadDraft('room', item.id);
  draftExpand.value = Boolean(content.value); draftPreview.value = false;
...
// 上传走原始 bytes（Content-Type 为图片 mime），会话 Cookie 自动携带鉴权，无需额外请求头
async function uploadGalleryFile(file) {
  if (!file) return;
  try {
    const resp = await fetch('/api/gallery', { method: 'POST', headers: { 'content-type': file.type || 'application/octet-stream' }, body: file });
...
function copyGalleryUrl(img) { copyText(new URL(img.url, location.origin).href, '外链已复制'); }
function sendGalleryImage(img) { content.value += `\n![](${img.url})\n`; galleryOpen.value = false; notify('已插入房间输入框'); }
...
async function retract(message) { if (!confirm('确定撤回这条消息吗？')) return; try { await api(`/api/messages/${message.id}`, { method: 'DELETE' }); message.deleted_at = new Date().toISOString(); notify('消息已撤回'); } catch (e) { notify(e.message); } }
async function viewOriginal(message, isDm) { try { const url = isDm ? `/api/dm/messages/${message.id}/original` : `/api/messages/${message.id}/original`; const result = await api(url); retractedOriginal.value = { ...result, message }; retractedOriginalOpen.value = true; } catch (e) { notify(e.message); } }
...
async function decideJoinRequest(userId, action) {
  try { await api(`/api/rooms/${room.value.id}/join-requests/${userId}/${action}`, { method: 'POST' }); await loadJoinRequests(); const members = await api(`/api/rooms/${room.value.id}/members`); roomMembers.value = members.members; notify(action === 'approve' ? '已通过' : '已拒绝'); } catch (e) { notify(e.message); }
}
...
async function selectConversation(conv) { saveDraft('dm', conversation.value?.id, dmInput.value); conversation.value = conv; dmMessages.value = []; dmLastId = 0; dmOldestId = 0; dmReplyTarget.value = null; view.value = 'dm'; room.value = null; roomPins.value = []; friendsOpen.value = false; dmInput.value = loadDraft('dm', conv.id); draftExpand.value = Boolean(dmInput.value); draftPreview.value = false; try { const result = await api(`/api/dm/conversations/${conv.id}/messages?before=9007199254740991&limit=60`); dmMessages.value = result.messages; dmLastId = dmMessages.value.at(-1)?.id || 0; dmOldestId = dmMessages.value[0]?.id || 0; await nextTick(); messageList.value?.scrollTo({ top: messageList.value.scrollHeight }); } catch (e) { notify(e.message); } await markConversationRead(); }
...
async function retractDm(message) { if (!confirm('确定撤回这条私信吗？')) return; try { await api(`/api/dm/messages/${message.id}`, { method: 'DELETE' }); message.deleted_at = new Date().toISOString(); notify('消息已撤回'); } catch (e) { notify(e.message); } }
...
onMounted(async () => {
  ...
  // 加载插件客户端资产（CSS/JS）
  loadPluginClientAssets();
  // 暴露 markdown 渲染函数给插件使用
  window.__polychat_markdown = (text) => markdown(text);
});
onBeforeUnmount(() => { shutdownRealtime(); document.removeEventListener('visibilitychange', handleVisibility); document.removeEventListener('scroll', onTourScroll, true); window.removeEventListener('keydown', handleGlobalShortcut); });
</script>
...
<template>
  ...
  <main v-else-if="!isMobile" class="chat">
    ...
    <aside :class="{open: sidebarOpen}"><header class="brand"><img :src="icon"><span>PolyChat<small>让交流保持简单</small></span></header><button class="new" @click="newRoom"><span>＋</span> 新建聊天室</button><div class="sidebar-search"><span>⌕</span><input v-model="sidebarSearchQuery" placeholder="搜索聊天室或私信…" type="text"></div><p class="nav-label">聊天室</p><nav><button v-for="item in filteredRooms" :key="item.id" :class="{active: view === 'rooms' && room?.id === item.id, hasUnread: unread[item.id], mentioned: mentionedUnread[item.id]}" @click="view = 'rooms'; choose(item); closeSidebar()"><span>#</span><b>{{ item.name }}</b><span v-if="item.is_private" class="room-badge">🔒</span><span v-if="item.locked" class="room-badge">🔐</span><span v-if="item.readonly" class="room-badge">📖</span><small v-if="mentionedUnread[item.id]" class="unread" style="background:#dc2626">@</small><small v-else-if="unread[item.id]" class="unread">{{ unread[item.id] > 99 ? '99+' : unread[item.id] }}</small></button></nav><p v-if="sidebarSearching && !filteredRooms.length" class="sidebar-empty">无匹配结果</p>
        <p class="nav-label">私信<button class="nav-add" title="好友与私信" @click="openFriends">＋</button></p>
        <nav><button v-for="conv in filteredConversations" :key="conv.id" :class="{active: view === 'dm' && conversation?.id === conv.id, hasUnread: dmUnread[conv.id]}" @click="selectConversation(conv); closeSidebar()"><span>✉</span><b>{{ conv.peer?.username || '私信' }}</b><small v-if="dmUnread[conv.id]" class="unread">{{ dmUnread[conv.id] > 99 ? '99+' : dmUnread[conv.id] }}</small></button></nav><p v-if="sidebarSearching && !filteredConversations.length" class="sidebar-empty">无匹配结果</p>
        <button class="sidebar-gallery" title="我的图床" @click="galleryOpen = true; loadGallery()"><span>🖼</span> 我的图床</button><footer class="sidebar-footer"><button class="profile-button" title="个人资料" @click="profileOpen = true"><img v-if="avatar(user)" :src="avatar(user)"><b v-else>{{ user.username[0] }}</b></button><span><b>{{ user.username }}<small class="user-number">#{{ user.number || user.id }}</small></b><small>{{ isAdmin ? '管理员 · 在线' : '在线' }}</small></span><button class="footer-icon" title="主题与自定义 CSS" @click="themeOpen = true">◐</button><button class="footer-icon" title="桌面通知" @click="toggleNotifications">{{ notificationOn ? '🔔' : '🔕' }}</button><button class="footer-icon" title="退出登录" @click="logout">↪</button></footer></aside>
    <section class="conversation">
      ...
      <div v-if="connectionState === 'reconnecting'" class="reconnect-bar">⚡ 正在重新连接…</div>
    ...
    <template v-else>
      ...
      <div class="m-home-body">
        ...
        <div v-else class="m-page">
          ...
          <button class="m-menu-item" @click="galleryOpen = true; loadGallery()"><span>🖼</span> 我的图床</button>
  ...
  <div v-if="profileOpen" class="modal"><section class="profile-modal"><button class="close" @click="profileOpen = false">×</button><p>YOUR PROFILE</p><h2>个人资料</h2><div class="avatar-preview"><img v-if="avatar(user)" :src="avatar(user)"><b v-else>{{ user.username[0] }}</b></div><h3>{{ user.username }}</h3><p class="hint">支持 PNG、JPEG、WebP、GIF，最大 2 MB</p><input ref="avatarInput" class="hidden-input" type="file" accept="image/png,image/jpeg,image/webp,image/gif" @change="setAvatar($event.target.files[0])"><div class="profile-actions"><button class="primary" @click="avatarInput.click()">选择新头像</button><button v-if="avatar(user)" @click="removeAvatar">移除头像</button></div><div class="data-actions"><h3>数据管理</h3><button @click="exportData">导出聊天记录</button><button @click="galleryOpen = true; loadGallery()">我的图床</button><button class="danger-button" @click="deleteAccount">删除账号</button><p class="hint">删除账号将永久移除所有消息和文件</p></div></section></div>
  <div v-if="galleryOpen" class="modal"><section class="gallery-modal"><button class="close" @click="galleryOpen = false">×</button><p>MY GALLERY</p><h2>我的图床</h2><p class="hint">上传的图片会保存在图床，可通过外链在任何地方引用；「发到聊天」会把 Markdown 图片语法插入到输入框。</p><div class="gallery-quota">已用 {{ galleryQuota.used_mb.toFixed(1) }} / {{ galleryQuota.quota_mb }} MB</div><input type="file" accept="image/png,image/jpeg,image/webp,image/gif" @change="e => { uploadGalleryFile(e.target.files[0]); e.target.value = ''; }"><div class="gallery-grid"><div v-for="img in galleryItems" :key="img.id" class="gallery-item"><img :src="img.url" :alt="img.filename" loading="lazy" @click="previewImage(img.url)"><div class="gallery-item-actions"><button @click="copyGalleryUrl(img)">复制外链</button><button @click="sendGalleryImage(img)">发到聊天</button><button class="danger" @click="deleteGalleryImage(img)">删除</button></div></div></div><div v-if="!galleryItems.length" class="modal-empty">图床还是空的，选一张图片上传吧。</div></section></div>
  ...
  <div v-if="retractedOriginalOpen && retractedOriginal" class="modal" @click.self="retractedOriginalOpen = false"><section class="retracted-modal"><button class="close" @click="retractedOriginalOpen = false">×</button><p>RETRACTED MESSAGE</p><h2>已撤回的消息</h2><div class="retracted-meta"><b>{{ retractedOriginal.username }}</b><small>· 原始内容</small></div><div class="markdown retracted-content" v-html="markdown(retractedOriginal.content || '（无文字内容）')"></div><div v-if="retractedOriginal.attachment_id" class="retracted-attachment"><a :href="`/api/files/${retractedOriginal.attachment_id}`" target="_blank">📎 查看附件</a></div></section></div>
