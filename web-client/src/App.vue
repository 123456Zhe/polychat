...
async function reviewBotRequest(id, status) { try { await api(`/api/admin/bot-requests/${id}`, { method: 'PUT', body: JSON.stringify({ status }) }); await loadBotRequests(); notify(status === 'approved' ? '已通过机器人申请' : '已拒绝机器人申请'); } catch (e) { notify(e.message); } }
...
<template>
    ...
    <section class="conversation">
      <header class="topbar">
        ...
        <div class="topbar-actions">
          ...
          <button v-if="isAdmin" class="toolbar-button" @click="adminOpen = true; loadAdmin(); loadBannedIps(); loadBannedFingerprints(); loadBotRequests(); adminTab = 'users'">管理面板</button>
  ...
  <div v-if="notifOpen" class="notif-dropdown">
    ...
    <div v-for="n in notifications" :key="n.id" class="notif-item" :class="{unread: !n.is_read}" @click="n.is_read || markNotifRead(n.id)">
      ...
      <div v-if="n.data?.token" class="notif-token"><span>Bot Token（请妥善保存）：</span><code>{{ n.data.token }}</code><button @click.stop="navigator.clipboard?.writeText(n.data.token); notify('Token 已复制')">复制</button></div>
    </div>
  </div>
</template>
