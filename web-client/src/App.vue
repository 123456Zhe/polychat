...
async function toggleReaction(message, emoji) { try { const result = await api(`/api/messages/${message.id}/reactions`, { method: 'POST', body: JSON.stringify({ emoji }) }); message.reactions = result.reactions; } catch (e) { notify(e.message); } }
...
async function loadMembers() { if (!room.value) return; try { roomMembers.value = (await api(`/api/rooms/${room.value.id}/members`)).members; membersOpen.value = true; } catch (e) { notify(e.message); } }
async function inviteMember() { const username = prompt('要邀请的用户名'); if (!username || !room.value) return; try { await api(`/api/rooms/${room.value.id}/members`, { method: 'POST', body: JSON.stringify({ username, role: 'member' }) }); await loadMembers(); notify('成员已邀请'); } catch (e) { notify(e.message); } }
async function newRoom() { const name = prompt('聊天室名称'); if (!name) return; const is_private = confirm('确定后创建私有聊天室？\n“取消”将创建公开聊天室。'); try { const result = await api('/api/rooms', { method: 'POST', body: JSON.stringify({ name, is_private }) }); await loadRooms(); await choose(result.room); } catch (e) { notify(e.message); } }
...
<template>
  ...
  <main v-else class="chat"><aside><header class="brand"><img :src="icon"><span>PolyChat<small>让交流保持简单</small></span></header><button class="new" @click="newRoom"><span>＋</span> 新建聊天室</button><p class="nav-label">聊天室</p><nav><button v-for="item in rooms" :key="item.id" :class="{active: room?.id === item.id, hasUnread: unread[item.id]}" @click="choose(item)"><span>#</span><b>{{ item.name }}</b><small v-if="unread[item.id]" class="unread">{{ unread[item.id] > 99 ? '99+' : unread[item.id] }}</small></button></nav><footer><button class="profile-button" title="更换头像" @click="profileOpen = true"><img v-if="avatar(user)" :src="avatar(user)"><b v-else>{{ user.username[0] }}</b></button><span>{{ user.username }}<small>{{ isAdmin ? '管理员 · 在线' : '在线' }}</small></span><button class="logout" title="退出登录" @click="logout">↪</button></footer></aside>
    <section class="conversation">
      ...
      <div ref="messageList" class="messages" @scroll.passive="maybeLoadOlderMessages"><p v-if="loadingOlderMessages" class="history-loading">正在加载更早消息…</p><p v-else-if="hasOlderMessages" class="history-hint">向上滚动加载更早消息</p><div v-if="!messages.length" class="empty"><img :src="icon"><h3>开始一段新对话</h3><p>发送 Markdown、公式、图片或文件。</p></div>
        <article v-for="message in messages" :key="message.id"><div class="avatar"><img v-if="avatar(message)" :src="avatar(message)"><b v-else>{{ message.username[0] }}</b></div><div class="bubble"><header><strong>{{ message.username }}</strong><small>{{ time(message.created_at) }}{{ message.edited_at ? ' · 已编辑' : '' }}</small><button @click="startReply(message)">回复</button><button title="复制原始 Markdown" @click="copy(message)">复制</button><button v-if="message.user_id === user.id && !message.deleted_at" @click="beginEdit(message)">编辑</button><button v-if="message.user_id === user.id || isAdmin || room?.role === 'owner' || room?.role === 'admin'" @click="retract(message)">撤回</button></header><blockquote v-if="message.reply_to" class="reply-reference">回复 {{ message.reply_username || '消息' }}：{{ message.reply_content || '已撤回的消息' }}</blockquote><p v-if="message.deleted_at" class="retracted">此消息已撤回</p><div v-else-if="message.content" class="markdown" v-html="markdown(message.content)"></div><template v-if="message.attachment_id"><img v-if="imageTypes.has(message.attachment_type)" class="attachment-image" :src="`/api/files/${message.attachment_id}?inline=1`" :alt="message.attachment_name"><a v-else class="attachment-file" :href="`/api/files/${message.attachment_id}`"><span>↓</span><div><b>{{ message.attachment_name }}</b><small>{{ size(message.attachment_size) }}</small></div></a></template><div v-if="!message.deleted_at" class="reactions"><button v-for="reaction in message.reactions" :key="reaction.emoji" :class="{active: reaction.reacted}" @click="toggleReaction(message, reaction.emoji)">{{ reaction.emoji }} {{ reaction.count }}</button><button v-for="emoji in ['👍','❤️','😂','🎉','😮','👀']" v-show="!message.reactions?.some(item => item.emoji === emoji)" :key="emoji" @click="toggleReaction(message, emoji)">{{ emoji }}</button></div></div></article>
      </div>
      <form class="composer" @submit.prevent="send"><div v-if="replyTarget" class="file-chip">↳ 回复 {{ replyTarget.username }}：{{ replyTarget.content?.slice(0, 80) }}<button type="button" @click="cancelReply">×</button></div><div v-if="file" class="file-chip"><span>{{ imageTypes.has(file.type) ? '图片' : '文件' }}</span>{{ file.name }}<button type="button" @click="file = null">×</button></div><div class="compose-row"><label class="attach" title="添加文件">＋<input ref="fileInput" type="file" @change="selectFile"></label><button type="button" class="attach emoji-trigger" title="EmojiAll 表情" @click="emojiOpen = !emojiOpen">☺</button><textarea v-model="content" rows="1" placeholder="输入消息，粘贴图片或使用 Markdown…" @paste="paste" @keydown.enter.exact.prevent="send"></textarea><button class="send" title="发送消息">发送</button></div><div v-if="emojiOpen" class="emoji-picker"><nav><button v-for="(_, category) in emojiGroups" :key="category" :class="{active: emojiCategory === category}" type="button" @click="emojiCategory = category">{{ category }}</button></nav><div><button v-for="emoji in emojiGroups[emojiCategory]" :key="emoji" type="button" :title="emoji" @click="insertEmoji(emoji)">{{ emoji }}</button></div></div><small>Enter 发送 · Shift + Enter 换行</small></form>
    </section></main>
  ...
  <div v-if="membersOpen" class="modal"><section><button class="close" @click="membersOpen = false">×</button><p>ROOM ACCESS</p><h2>房间成员</h2><button class="primary" @click="inviteMember">邀请成员</button><div class="member" v-for="member in roomMembers" :key="member.id"><span>{{ member.username }}</span><small>{{ member.role }}</small></div></section></div>
</template>
