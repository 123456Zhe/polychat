...
<template>
  ...
  <div v-if="galleryOpen" class="modal"><section class="gallery-modal"><button class="close" @click="galleryOpen = false">×</button><p>MY GALLERY</p><h2>我的图床</h2><p class="hint">上传的图片会保存在图床，可通过外链在任何地方引用；「发到聊天」会把 Markdown 图片语法插入到输入框。</p><div class="gallery-quota">已用 {{ galleryQuota.used_mb.toFixed(1) }} / {{ galleryQuota.quota_mb }} MB</div><input type="file" accept="image/png,image/jpeg,image/webp,image/gif" @change="e => uploadGalleryFile(e.target.files[0])"><div class="gallery-grid"><div v-for="img in galleryItems" :key="img.id" class="gallery-item"><img :src="img.url" :alt="img.filename" loading="lazy" @click="previewImage(img.url)"><div class="gallery-item-actions"><button @click="copyGalleryUrl(img)">复制外链</button><button @click="sendGalleryImage(img)">发到聊天</button><button class="danger" @click="deleteGalleryImage(img)">删除</button></div></div></div><div v-if="!galleryItems.length" class="modal-empty">图床还是空的，选一张图片上传吧。</div></section></div>
