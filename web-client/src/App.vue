...
async function banUserIp(member) { if (!member.last_ip) return notify('该用户没有记录 IP'); const hours = prompt('封禁小时数（留空为永久）：'); const duration = hours ? Number(hours) : null; try { await api('/api/admin/banned-ips/ban', { method: 'PUT', body: JSON.stringify({ ip: member.last_ip, duration_hours: duration, reason: `用户 ${member.username} 的 IP` }) }); await loadBannedIps(); notify(`已封禁 ${member.username} 的 IP ${member.last_ip}`); } catch (e) { notify(e.message); } }
...
<template>
  ...
  <div v-if="adminOpen" class="modal"><section class="admin-modal"><button class="close" @click="adminOpen = false">×</button><p>ADMINISTRATION</p><h2>管理面板</h2><div class="stats"><span v-for="(value, key) in admin.stats" :key="key"><b>{{ value }}</b>{{ {users:'用户', rooms:'聊天室', messages:'消息', files:'文件'}[key] }}</span></div><h3>用户 <button @click="loadAdmin">刷新</button></h3><div v-for="member in admin.users" :key="member.id" class="member"><div class="member-info"><span>{{ member.username }} · {{ member.message_count }} 条消息 <small v-if="member.last_ip" class="ip-badge">{{ member.last_ip }}</small></span><span v-if="member.banned_until" class="status-badge banned">封禁至 {{ new Date(member.banned_until).toLocaleString() }}</span><span v-if="member.muted_until" class="status-badge muted">禁言至 {{ new Date(member.muted_until).toLocaleString() }}</span></div><div class="member-actions"><button @click="toggleAdmin(member)">{{ member.is_admin ? '撤销管理员' : '设为管理员' }}</button><template v-if="!member.is_admin"><button v-if="!member.banned_until" @click="banUser(member)">封禁</button><button v-else @click="unbanUser(member)">解封</button><button v-if="!member.muted_until" @click="muteUser(member)">禁言</button><button v-else @click="unmuteUser(member)">解除禁言</button><button @click="banUserIp(member)">封禁 IP</button></template></div></div><h3>IP 封禁 <button @click="loadBannedIps">刷新</button></h3><form class="ban-ip-form" @submit.prevent="adminBanIp"><input v-model="banIpInput" placeholder="输入 IP 地址" class="modal-input"><select v-model="banIpDuration"><option :value="null">永久</option><option :value="1">1 小时</option><option :value="24">24 小时</option><option :value="168">7 天</option><option :value="720">30 天</option></select><button class="primary">封禁</button></form><div v-for="entry in bannedIps" :key="entry.ip_address" class="banned-ip"><span class="code-value">{{ entry.ip_address }}</span><small v-if="entry.banned_until">至 {{ new Date(entry.banned_until).toLocaleString() }}</small><small v-else>永久</small><small v-if="entry.reason">{{ entry.reason }}</small><small v-if="entry.admin_name">by {{ entry.admin_name }}</small><button @click="adminUnbanIp(entry.ip_address)">解封</button></div></section></div>
