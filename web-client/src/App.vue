...
<template>
  ...
  <main v-else class="chat">
    ...
    <section class="conversation">
      ...
      <form v-if="view === 'dm' && conversation" class="composer" @submit.prevent="sendDm"><div v-if="dmReplyTarget" class="file-chip">↳ 回复 {{ dmReplyTarget.username }}：{{ dmReplyTarget.content?.slice(0, 80) }}<button type="button" @click="dmReplyTarget = null">×</button></div><div v-if="dmFiles.length" class="file-chips"><div v-for="(f, index) in dmFiles" :key="index" class="file-chip"><img v-if="imageTypes.has(f.type)" :src="filePreview(f)" class="file-preview"><span>{{ imageTypes.has(f.type) ? '图片' : '文件' }}</span>{{ f.name }}<button type="button" @click="dmFiles.splice(index, 1)">×</button></div></div><div class="compose-row"><label class="attach" title="添加文件">＋<input ref="dmFileInput" type="file" multiple @change="dmFiles = [...dmFiles, ...Array.from($event.target.files)]"></label><button type="button" class="attach emoji-trigger" title="表情" @click="emojiOpen = !emojiOpen">☺</button><textarea v-model="dmInput" rows="1" :placeholder="'私信给 ' + (conversation.peer?.username || '好友') + '…'" @keydown.enter.exact.prevent="sendDm"></textarea><button class="send" title="发送">发送</button></div><div v-if="emojiOpen" class="emoji-picker"><nav><button v-for="(_, category) in emojiGroups" :key="category" :class="{active: emojiCategory === category}" type="button" @click="emojiCategory = category">{{ category }}</button></nav><div><button v-for="emoji in emojiGroups[emojiCategory]" :key="emoji" type="button" :title="emoji" @click="dmInput += emoji; emojiOpen = false">{{ emoji }}</button></div></div><small>Enter 发送 · Shift + Enter 换行 · 私信仅双方可见</small></form>
