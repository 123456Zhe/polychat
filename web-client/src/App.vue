...
const view = ref('rooms'), conversations = ref([]), conversation = ref(null), dmMessages = ref([]), dmLastId = 0, dmOldestId = 0, dmUnread = ref({});
...
async function handleSocketEvent(event) {
  ...
  if (event.type === 'announcement') { if (room.value?.id === event.room_id) await loadRooms(); return; }
  ...
  if (event.type === 'message') {
    if (event.message && room.value?.id === event.room_id) { messages.value = appendUnique(messages.value, [event.message]); lastId = event.message.id || lastId; await nextTick(); messageList.value?.scrollTo({ top: messageList.value.scrollHeight, behavior: 'smooth' }); }
...
<template>
  ...
  <main v-else class="chat">
    ...
    <aside :class="{open: sidebarOpen}"><header class="brand"><img :src="icon"><span>PolyChat<small>让交流保持简单</small></span></header><button class="new" @click="newRoom"><span>＋</span> 新建聊天室</button><p class="nav-label">聊天室</p><nav><button v-for="item in rooms" :key="item.id" :class="{active: view === 'rooms' && room?.id === item.id, hasUnread: unread[item.id]}" @click="view = 'rooms'; choose(item); closeSidebar()"><span>#</span><b>{{ item.name }}</b><small v-if="unread[item.id]" class="unread">{{ unread[item.id] > 99 ? '99+' : unread[item.id] }}</small></button></nav>
        ...
        <footer><button class="profile-button" title="更换头像" @click="profileOpen = true"><img v-if="avatar(user)" :src="avatar(user)"><b v-else>{{ user.username[0] }}</b></button><span>{{ user.username }}<small>{{ isAdmin ? '管理员 · 在线' : '在线' }}</small></span><button class="logout" title="退出登录" @click="logout">↪</button></footer></aside>
    <section class="conversation">
      ...
      <form v-if="view === 'dm' && conversation" class="composer" @submit.prevent="sendDm"><div v-if="dmReplyTarget" class="file-chip">↳ 回复 {{ dmReplyTarget.username }}：{{ dmReplyTarget.content?.slice(0, 80) }}<button type="button" @click="dmReplyTarget = null">×</button></div><div v-if="dmFiles.length" class="file-chips"><div v-for="(f, index) in dmFiles" :key="index" class="file-chip"><img v-if="imageTypes.has(f.type)" :src="filePreview(f)" class="file-preview"><span>{{ imageTypes.has(f.type) ? '图片' : '文件' }}</span>{{ f.name }}<button type="button" @click="dmFiles.splice(index, 1)">×</button></div></div><div class="compose-row"><label class="attach" title="添加文件">＋<input ref="dmFileInput" type="file" multiple @change="dmFiles = [...dmFiles, ...Array.from($event.target.files)]"></label><button type="button" class="attach emoji-trigger" title="表情" @click="emojiOpen = !emojiOpen">☺</button><textarea v-model="dmInput" rows="1" placeholder="私信给 {{ conversation.peer?.username }}…" @keydown.enter.exact.prevent="sendDm"></textarea><button class="send" title="发送">发送</button></div><div v-if="emojiOpen" class="emoji-picker"><nav><button v-for="(_, category) in emojiGroups" :key="category" :class="{active: emojiCategory === category}" type="button" @click="emojiCategory = category">{{ category }}</button></nav><div><button v-for="emoji in emojiGroups[emojiCategory]" :key="emoji" type="button" :title="emoji" @click="dmInput += emoji; emojiOpen = false">{{ emoji }}</button></div></div><small>Enter 发送 · Shift + Enter 换行 · 私信仅双方可见</small></form>
