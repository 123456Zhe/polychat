...
`.attachment-image` 与 `.markdown img` 默认 `max-width: 100%`、`height: auto` 和 `max-height: 70vh`：宽图会缩小到消息气泡内，窄图不会被放大，图片始终保持纵横比。若需要进一步限制图片尺寸，可覆盖：

```css
.attachment-image, .markdown img { max-width: min(100%, 900px); height: auto; }
```
