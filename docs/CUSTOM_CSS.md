...
| `.attachment-image` | 等比例缩放到消息气泡宽度内的图片附件 |
| `.markdown img` | Markdown `![]()` 语法生成的图片，使用与附件相同的缩放规则 |
...
`.attachment-image` 与 `.markdown img` 默认 `max-width: 100%`、`height: auto` 和 `max-height: 70vh`：宽图会缩小到消息气泡内，窄图不会被放大，图片始终保持纵横比。该规则在消息气泡内使用 `!important`，以避免 Markdown 图片的 HTML 尺寸属性或旧的自定义主题将图片撑破布局。若需要进一步限制图片尺寸，请使用同等的 `!important` 覆盖：

```css
.bubble .attachment-image,
.bubble .markdown img { max-width: min(100%, 900px) !important; height: auto !important; }
```
