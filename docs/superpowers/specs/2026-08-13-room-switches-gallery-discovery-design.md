...
- **qiniu 域名类型**：自定义域名分「CDN 加速域名」与「源站域名」两种，官方均要求 ICP 备案，但备案巡检只针对**中国大陆区域空间**——海外区域（`na0`/`as0`）空间的源站域名实际免备案；源站域名无 CDN 缓存、国内访问速度一般，适合图床外链；CDN 加速域名需备案且适合正式生产。测试域名有效期 30 天且仅限对接测试，不可用于生产。
